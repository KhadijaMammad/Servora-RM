...
export const Tables = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 1. Statusu string kimi alırıq, sonra rəqəmə (number) çeviririk
    const statusValue = Number(formData.get("status"));

    if (selectedTable) {
      // Əsas məlumatları yenilə
      await updateTable({ id: selectedTable.id, body: basePayload }).unwrap();

      // 2. Status dəyişibsə, rəqəm formatında göndər
      // selectedTable.status-un da rəqəm olduğunu nəzərə al
      if (statusValue !== selectedTable.status) {
        await updateStatus({
          id: selectedTable.id,
          status: statusValue,
        }).unwrap();
      ...
      await addTable({ ...basePayload, status: 0 }).unwrap();
    }
  ...
  const handleStatusChange = async (id: string, statusValue: number) => {
    console.log("Göndərilən Status (rəqəm olaraq):", statusValue);
    await updateStatus({ id, status: statusValue }).unwrap();
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-blue-950/20 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <form
            ...
            className="bg-white p-8 rounded-4xl w-full max-w-sm shadow-2xl space-y-4"
          >
            ...
            <input
              name="tableNumber"
              defaultValue={selectedTable?.tableNumber}
            ...
            {selectedTable && (
              <select
                ...
                onChange={(e) =>
                  handleStatusChange(selectedTable.id, Number(e.target.value))
                }
                className="w-full p-4 bg-slate-50 rounded-2xl"
              >
            ...
            <div className="flex gap-2 pt-4">
              ...
              {selectedTable && (
                <button
                  ...
                  className="px-6 py-4 bg-red-100 text-red-600 rounded-2xl"
                >
                  Delete
                </button>
              )}
